...
require-with = "warn"       # severity for tyc::resource_not_managed (open()/socket()/connect() bound outside `with`)
blocking-in-async = "warn"  # severity for tyc::blocking_in_async (time.sleep / requests.get / subprocess.run inside async def)
suggest-gather = true       # advice: surface tyc::gather_opportunity on adjacent independent awaits
...
| `traceback-remap` | bool (default `false`) | When `true`, inject `typhon_runtime.traceback.install()` into the entry module's `if __name__ == "__main__":` block so an uncaught exception's traceback is rewritten to point at `.ty` source automatically (the same mapping `tyc trace` applies), with no manual step. When the entry module has no guard — a script that just calls `main()` at the bottom — one is synthesised after the docstring and imports, ahead of the first executable statement, so the knob is never silently inert. Every row is rewritten, not just the `File "…", line N` header: the source row under each frame is replaced with the real `.ty` row and the column anchors are dropped (they no longer line up); where the `.ty` cannot be read, the original row is kept. Only the entry script is affected — library imports never trip the `__main__` guard — and the hook falls back to the previous behaviour on any failure, so it can only improve a traceback. Default-off keeps existing projects and runtime-free entry points dependency-free. |
...
| `require-with` | `"error"` \| `"warn"` \| `"off"` | Severity for `tyc::resource_not_managed` — a call to a known resource-returning function (`open`, `socket.socket`, `sqlite3.connect`, `tempfile.NamedTemporaryFile` / `TemporaryDirectory` / `TemporaryFile`) bound to a variable outside a `with` statement, which leaves cleanup to the garbage collector. `"warn"` (default) keeps it visible without breaking CI; `"error"` promotes it once the migration is paid down; `"off"` drops it. `@contextmanager` / `@asynccontextmanager` factory bodies are exempt. |
| `blocking-in-async` | `"error"` \| `"warn"` \| `"off"` | Severity for `tyc::blocking_in_async` — a direct call to a known-blocking stdlib function (`time.sleep`, `requests.get` / `post` / …, `urllib.request.urlopen`, `subprocess.run` / `call` / `check_call` / `check_output`, `input`, `socket.recv`) inside an `async def`, which stalls the event loop. `"warn"` (default) surfaces it without breaking CI; `"error"` promotes it for codebases that have finished moving to async-aware libraries (`httpx`, `asyncio.sleep`, `asyncio.to_thread(...)`); `"off"` suppresses it. Suppressed inside `unsafe:` blocks. |
| `suggest-gather` | bool | When `true` (the default), `tyc check` / `tyc build` / the LSP surface the advice-level `tyc::gather_opportunity` for every run of 2+ adjacent independent awaited calls inside an `async def` — including awaited method calls on imported clients, which `auto-gather` never touches — suggesting an explicit `gather:` block. Advice-only: it never rewrites and never blocks a build. Set `false` to silence it. |
| `auto-memoise` | bool | Whether to apply `@functools.cache` to functions the analyser infers as pure. Default `false`, but **defaults to `true` at `[optimise] level = 1`** (an explicit entry here always wins). Caches are *never* inserted silently: even when enabled, the analyser requires the function to be **provably** pure — every call resolved to a pure builtin, a same-module `@pure` helper, or the pure stdlib allow-list, and every read to an immutable binding — with an immutable return type and immutable, hashable parameters (see [language.md — What the verifier proves](language.md#what-the-verifier-proves)). A function the analyser merely cannot prove impure (an unknown method call, a third-party module) is left alone. |
| `auto-gather` | bool | When `true`, runs of two-or-more consecutive independent `name = await callee(...)` statements inside an `async def` are folded into an `asyncio.TaskGroup` so they execute concurrently. A callee name the enclosing function rebinds (a parameter, a local, a nested `def`, an import) is never folded — it refers to a different callable — nor is one rebound at module level after its definition; runs lexically inside a `try` or `with` body stay sequential because a `TaskGroup` re-raises as an `ExceptionGroup`, which a same-frame `except ValueError` would no longer catch. Independence is decided by static data-flow on bound names; **every callee must be a same-module `async def` carrying an explicit `@gatherable` decorator** — undecorated async functions and externally-imported callees are left alone, so opting a project into `auto-gather` does not surprise callers that aren't ready to run concurrently. Default `false` (defaults to `true` at `[optimise] level = 1`; explicit entry wins). Explicit `gather:` blocks are unaffected. |
...
| `pgo-memoise` | bool | When `true`, `tyc build` reads `typhon-profile.json` from the project root (the directory holding `typhon.toml`; produced by running a `tyc profile`-instrumented build from that directory) and promotes every top-level function that is provably pure with a cache-safe signature (the same bar as `auto-memoise`) and whose observed call count meets `pgo-min-calls` to `@functools.cache`, even if the user did not write `@memo`. Profile keys are matched per module — `users.find_user` promotes `find_user` in `src/users.ty` only; the entry module is recorded as `__main__.<fn>`, which is accepted alongside `main.<fn>` for `src/main.ty`, so the default layout round-trips. Complements `auto-memoise` (which caches every pure function regardless of profile data). Missing profile file is not an error (nor a warning) — PGO is best-effort. Default `false` (defaults to `true` at `[optimise] level = 1`; explicit entry wins). See [`tyc profile`](cli.md#tyc-profile). |
...
| `nullable-use` | `"warn"` \| `"error"` \| `"off"` | Severity for the *attribute-rooted* form of `tyc::nullable_use` — dereferencing a possibly-`None` field (`self.db.host` where `db: Db?`). `"warn"` (default) is the introduction severity: the check never ran before v1.0.0-alpha.7, so an immediate error would reject programs whose nullable field happens always to be populated. `"error"` promotes it (the documented migration path); `"off"` suppresses it. The bare-name form (`x.upper()` where `x: str?`) has always been an error and is not governed by this knob. |
...
| `allow-secret-comptime` | bool | When `true`, silences `tyc::contains_secret_literal` — the warning that fires when a `comptime let` binding whose name looks like a secret (`*KEY` / `*TOKEN` / `*PASSWORD` / `*SECRET` / `*PASS` / `*PWD` and the rest of the 55-entry keyword table as of v1.0.0-alpha.9) would inline the resolved env-var value as a string literal into the build artifact. Default `false`; `tyc init` seeds it explicitly. Prefer reading secrets at runtime via `os.environ[...]` over enabling this. |
