...
| `auto-parallel` | bool | When `true`, list comprehensions whose element is a pure call are rewritten at build time into a thread-pool map. Combine with `[python] free-threaded` to release the GIL across workers; on stock CPython the rewrite still runs but the GIL serialises workers. Default `false` (defaults to `true` at `[optimise] level = 1`; explicit entry wins). |
| `auto-parallel-reductions` | bool | When `true`, integer accumulator loops (`for x in xs: total += f(x)`) over a pure body are also eligible for the parallel-reduction rewrite. **Requires `auto-parallel` to have any effect** — it extends that same build-time pass from pure comprehensions to accumulator loops. Default `false`. |
...
| `parallel-backend` | `"threads"` \| `"interpreters"` | Execution backend for auto-parallel rewrites. `"threads"` (default) uses a `concurrent.futures.ThreadPoolExecutor`; `"interpreters"` targets PEP 734 sub-interpreters. Any other value is rejected at config load rather than silently falling back to threads. |
...
| `suggest-parallel` | bool | When `true` (the default), surface `tyc::parallel_opportunity` advice on a loop or comprehension that could be parallelised (see `auto-parallel`). Advice-only; never blocks a build. Set `false` to silence it. |
