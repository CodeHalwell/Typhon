...
use ttc_resolve::{Binding, BindingKind, ResolvedModule, ScopeId};
...
impl<'a> Checker<'a> {
    ...
    fn mismatch(&mut self, expected: &Type, actual: &Type, span: (usize, usize)) {
        ...
        self.diagnostics.push_error(TtcError::type_mismatch(
            expected.display(),
            actual.display(),
            &self.path,
            self.source,
    ...
    fn nullable_use(&mut self, name: &str, expected: &Type, span: (usize, usize)) {
        let length = span.1.saturating_sub(span.0).max(1);
        self.diagnostics.push_error(TtcError::nullable_use(
            name,
    ...
    fn wrong_args(&mut self, name: &str, expected: usize, actual: usize, span: (usize, usize)) {
        let length = span.1.saturating_sub(span.0).max(1);
        self.diagnostics.push_error(TtcError::wrong_arg_count(
            name,
            expected,
            actual,
            &self.path,
    ...
    fn not_callable(&mut self, typ: &Type, span: (usize, usize)) {
        let length = span.1.saturating_sub(span.0).max(1);
        self.diagnostics.push_error(TtcError::not_callable(
            typ.display(),
            &self.path,
            self.source,
            span.0,
