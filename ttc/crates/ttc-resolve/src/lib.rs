...
use ttc_syntax::lexer::TyphonKeyword;
use ttc_syntax::preprocess::StrippedKeyword;
    ...
    fn declare(
        ...
        if let Some(existing) = self.lookup_local(scope, name) {
            if existing.mutability == Mutability::Val && kind == BindingKind::Value {
                ...
                self.diagnostics.push_error(TtcError::immutable_assign(
                    name,
                    &self.path,
                    self.source,
                    decl_span.0,
    ...
    fn report_unknown_names(&mut self) {
        ...
        for r in &self.references {
            ...
            if !found && !builtins.contains(&r.name.as_str()) {
                ...
                self.diagnostics.push_error(TtcError::unknown_name(
                    r.name.clone(),
                    &self.path,
                    self.source,
                    r.span.0,
