...
## 2024-11-26 - Zero-Allocation in `tyc-analyse` parallel lints
**Learning:** Checking for string equality against AST node identifiers using `HashSet<String>` in `tyc-analyse/src/parallel_lints.rs` (like `module_level_mut_names`, `collect_module_mut_names`, `collect_globals`, and `fn_writes_shared_state`) creates unnecessary heap allocations and overhead. Furthermore, `ruff_python_ast` nodes like `Name` and `Identifier` implement `.as_str()` method, which can be used to easily borrow string slices.
**Action:** Use `.as_str()` on `ruff_python_ast` nodes to borrow string slices directly from the AST during traversal instead of calling `.to_string()`. This eliminates heap allocations for short-lived collections by allowing the use of `HashSet<&str>`. Always verify the fields and methods of external types (e.g., using `grep` on the vendor directory or running `cargo check`) before planning a refactor to ensure groundedness.
