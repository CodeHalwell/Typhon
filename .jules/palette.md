...
## 2024-05-16 - Ensure missing transition properties match hover state
**Learning:** Found that custom components `.card` and `.sl-link-card` (from Starlight) lacked transition properties on their base state for attributes they animate on hover (such as `box-shadow`, `background-color`, `transform`, and `.icon` `color`), resulting in jarring and abrupt animations. This means when adding custom hover/focus states to any components, we must ensure their base classes have corresponding `transition` settings.
**Action:** Added matching `transition: [...] 120ms ease;` declarations for `.card` (added `box-shadow`), `.sl-link-card` (added `background-color`, `border-color`, `transform`, `box-shadow`), and `.sl-link-card .icon` (`color`) to ensure animations trigger smoothly when interacting with these components.
