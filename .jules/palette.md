...
## 2024-05-19 - Third-Party Component Transitions
**Learning:** When customizing hover or focus states (like `box-shadow` or `transform`) on third-party components (like Astro Starlight's `.sl-link-card`), it's common that the original component lacks a generic base `transition` property for those specific css properties if they didn't originally animate them.
**Action:** Always verify and append base `transition` properties in `custom.css` alongside any custom pseudo-class style changes to ensure smooth UX animations instead of abrupt snapping.
