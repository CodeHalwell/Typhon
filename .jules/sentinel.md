...
## 2024-05-30 - Expanded Secret Keyword Detection
**Vulnerability:** The hardcoded secret literal detector (`tyc::contains_secret_literal`) missed multiple critical credential-indicating keywords like `WEBHOOK`, `CREDENTIALS`, `DSN`, and most notably `PASSPHRASE` due to strict bounding constraints originally meant to prevent false positives on keywords like `PASS`.
**Learning:** Hardcoded keyword boundary logic that relies purely on suffixes (or strict boundaries meant to disambiguate short words like `KEY` vs `MONKEY` or `PASS` vs `PASSPORT`) is too rigid and actively creates false assurance by silently ignoring high-risk compound identifiers if they don't perfectly align with assumed word boundaries.
**Prevention:** When defining security heuristics, aggressively expand the base dictionary with all standard credential-bearing suffixes and strings (`AUTHORIZATION`, `WEBHOOK`, `PRIVKEY`), ensuring they are ordered longest-first so the scanner can reliably identify them without getting tripped up by short-word boundary rules.
