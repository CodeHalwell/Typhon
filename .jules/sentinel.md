...
## 2026-08-10 - [Expand Hardcoded Secret Detection Keywords]
**Vulnerability:** The static analysis secret detector (`SECRET_NAME_KEYWORDS` in `tyc-analyse`) was missing several high-impact secret name heuristics such as `CREDENTIALS`, `WEBHOOK`, `COOKIE`, `DSN`, `SIGNING`, and `AUTHORIZATION`, causing it to miss realistic hardcoded leaks (e.g. `SLACK_WEBHOOK_URL`). The documentation for `is_secret_name` was also inaccurately describing a suffix-match, rather than the bounded substring search actually implemented.
**Learning:** Security heuristics often drift from real-world usage patterns if not routinely updated to include common variants (like `DSN` or `WEBHOOK`). Docstrings describing security logic also need careful synchronization with their implementations.
**Prevention:** Regularly review false-negatives in secret detection and continually expand keyword dictionaries while ensuring documentation accurately reflects the underlying detection mechanism (e.g. suffix vs. bounded substring).
