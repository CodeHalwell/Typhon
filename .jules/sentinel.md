...
## $(date +%Y-%m-%d) - Ensure hardcoded secret detection is comprehensive
**Vulnerability:** First-tier secret names like `CREDENTIALS`, `AUTHORIZATION`, `WEBHOOK`, `COOKIE`, `DSN`, and `SIGNING` were not flagged by `tyc::contains_secret_literal` because their substrings were missing from `SECRET_NAME_KEYWORDS`.
**Learning:** Hardcoded secrets detection often prioritizes specific terms (like `PASSWORD` or `TOKEN`) while missing common patterns involving connection strings or broader security concepts like credentials.
**Prevention:** Regularly review and extend static secret-scanning keyword lists to include all standard environment variable names that typically store secrets, maintaining longest-first ordering.
