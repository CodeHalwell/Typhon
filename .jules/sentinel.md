...
## $(date +%Y-%m-%d) - Ensure cleanup of scratchpad files before commit
**Vulnerability:** Personal testing scratchpad files (e.g. `test_secret.rs`) and their compiled binaries were inadvertently included in the repository root alongside the actual fix.
**Learning:** Checking compiled binaries or temporary testing files into version control pollutes the repository and poses potential security and operational risks (e.g. executing untrusted code or bloated histories).
**Prevention:** Always run `git status` or `ls` after finalizing a fix, and explicitly remove any temporary scratchpads, debug logs, or generated binaries before initiating code reviews or PR creation.
