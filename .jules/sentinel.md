...
## $(date +%Y-%m-%d) - Ensure complete coverage of secret-related keywords
**Vulnerability:** The hardcoded secrets check (`contains_secret_literal`) missed multiple common secret-related keywords such as `ACCESS_TOKEN`, `BEARER_TOKEN`, `PRIVATE_KEY`, `CLIENT_SECRET`, `CSRF_TOKEN`, `JWT_TOKEN`, and `AUTH_TOKEN`, allowing hardcoded credentials matching these names to bypass detection.
**Learning:** Security scanning keyword lists must be comprehensive and include standard industry conventions for secret names (e.g., OAuth, JWT, CSRF, TLS) to prevent obvious false negatives.
**Prevention:** Regularly review and expand secret scanning keyword dictionaries. When adding them to arrays reliant on longest-first substring matching (like `tyc_analyse::SECRET_NAME_KEYWORDS`), strictly insert the new keywords in longest-first order to prevent partial match masking (e.g., ensuring `CLIENT_SECRET` precedes `SECRET`).
