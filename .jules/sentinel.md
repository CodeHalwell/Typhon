...
## $(date +%Y-%m-%d) - Ensure all permutations of high-risk security keywords are explicitly registered
**Vulnerability:** A hardcoded secrets check (`contains_secret_literal`) failed to detect `DBPASSWORD`, `JWTTOKEN`, and their snake_case variants as secrets, despite catching `PASSWORD` and `TOKEN`.
**Learning:** Case-boundary heuristics and substring matching logic miss fully capitalized squashed acronyms like `DBPASSWORD` if they are not explicitly registered as standalone keywords, leading to false negatives for common secret variable names.
**Prevention:** Explicitly register all common permutations of high-risk security keywords (like `DBPASSWORD`, `DB_PASSWORD`, `JWT_TOKEN`, `JWTTOKEN`) in the keyword dictionary.
