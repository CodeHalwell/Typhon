...
def envelope_from_json(raw: str) -> JobEnvelope | None:
    try:
        ...
        if kind == "resize":
            job = ResizeJob(
                image_path=str(data["image_path"]), width=int(data["width"])
            )
        elif kind == "email":
            job = EmailJob(
                to=str(data["to"]), subject=str(data["subject"]), body=str(data["body"])
            )
        elif kind == "report":
...
async def enqueue(r: aioredis.Redis, queue: str, job: Job) -> str:
    env: JobEnvelope = JobEnvelope(
        id=uuid.uuid4().hex, enqueued_at=time.time(), job=job
    )
...
async def main_async() -> None:
    ...
    async with asyncio.TaskGroup() as __typhon_tg_0__:
        __typhon_gather_1__ = __typhon_tg_0__.create_task(
            worker(r, queue, "worker-A", max_jobs=2)
        )
        __typhon_gather_2__ = __typhon_tg_0__.create_task(
            worker(r, queue, "worker-B", max_jobs=2)
        )
