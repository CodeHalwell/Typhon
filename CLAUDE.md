...
Current release: **v1.0.0-alpha** (workspace version in `tyc/Cargo.toml`). The language is **additive across the v0.3.0 → v1.0.0-alpha line** — every previously-accepted program must continue to type-check identically. Treat this as a hard compatibility constraint when changing the type checker.
