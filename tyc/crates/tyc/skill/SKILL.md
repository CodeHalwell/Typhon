...
- `extend BUILTIN:` (`str`, `list`, `int`, `dict`, …) extracts each method to a module-level free function `__typhon_ext_<TYPE>__<METHOD>`, and rewrites `x.method(...)` to the free-function call **whenever the receiver `x` is statically annotated as that built-in**. No monkey-patching — un-annotated receivers still raise `AttributeError` at runtime. `extend list[int]:` (parametric target) fires `tyc::extend_builtin` — drop the brackets. **`extend BUILTIN:` is module-local** (unlike `extend ClassName:`, which merges into a user class and crosses modules): the static rewrite only fires inside the declaring module, so importing that module does *not* carry the extension — `title.slug()` in a consumer fires `tyc::attribute_not_found` on `str`. Share the behaviour by wrapping it in a `pub def` free function and importing *that*.
