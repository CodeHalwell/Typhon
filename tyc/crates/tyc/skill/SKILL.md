...
- **`examples/NN-*/`** — 32 curated single-file exercises (sparsely numbered `01`–`68`, leaving room between topics). Most are stdlib-only; `06` and `17` need `pydantic` (they use `model`), and `28` / `29` / `33` / `38` / `40` / `43` need their named third-party package to *run* (all 32 type-check and build without it). `47-mini-app/` is a multi-file project rather than a single exercise.
...
- **`examples/parity/`** — output-deterministic programs asserted byte-identical under `tyc run` and `tyc build` + CPython (the drop-in guarantee), plus `parity/divergent/` recording the confirmed VM bugs. Check here before assuming a `tyc run` result matches the shipped build.
- **`examples/errors/`** — 71 deliberately-broken programs covering 56 of the 87 emittable codes, one per rule, each with a header comment naming its diagnostics and showing the fix. Twelve themed directories plus `11-multi-error-programs/` (realistic files with many errors at once, and which errors mask others) and `12-known-gaps/` (programs that compile clean and still fail at runtime). Consult `12-known-gaps/` before trusting a clean `tyc check` in the areas it names. Reach for these when explaining *why* a rule exists — the diagnostic is the explanation. Each file's expected codes are asserted by `tyc/crates/tyc/tests/error_examples.rs`.
- The full example corpus is **259 `.ty` files** (33 in the numbered exercises + `examples/testing/`, 221 across the 15 apps, 5 in `47-mini-app/`). Every one of them is `tyc check`-clean and `tyc build`-clean on the current release. The 33 single-file exercises each ship a checked-in `.py` companion showing the lowering; the multi-file apps do not (build them to see their output).
...
Inference is bidirectional and recursive; `pair(1, "two")` for `pair[T](a: T, b: T)` widens `T` to `int | str`. **Never import `TypeVar` from `typing`** — that path is a hard **error** (`tyc::typevar_import_rejected`). The deprecated capitalised aliases (`List`, `Dict`, `Tuple`, `Set`, `FrozenSet`, `Type`) behave differently from what the two warn-level codes suggest: importing one warns (`tyc::typing_alias_deprecated`), using it in an annotation warns again (`tyc::typing_alias_in_annotation`) — **but the checker also treats `Dict[K, V]` as an opaque foreign class rather than unifying it with `dict[K, V]`, so every literal and call site then fails with `tyc::type_mismatch` ("expected `Dict[str, int]`, found `dict[?, ?]`").** The build stops on that, not on the warnings. Treat the aliases as unusable, not merely discouraged, and use the lowercase built-ins.
...
Stacks with every modifier: `pub class Foo frozen:` (the `frozen` marker stays *after* the class name — `pub frozen class Foo:` does not parse), `pub model`, `pub let`, `pub mut`, `pub freeze let`, `pub newtype`, `pub interface`, `pub type`, `pub def`, `pub async def`, and `pub comptime let` / `pub comptime mut` / `pub comptime def` (v0.15.4). The one current exception is **`pub lazy let`, which does NOT parse** — see §7.
...
The v0.3.0 → v1.0.0-alpha.6 line is **additive on *correct* programs**. Every program that type-checked *and ran correctly* continues to behave identically; the new language forms in the whole window are the **`enum` keyword** (v0.11.0), the **`as!` checked boundary cast** (v0.14.0, made to compose everywhere in v0.15.0), and the **`rescue` exception-boundary sugar** (v1.0.0-alpha). Three deliberate narrowings exist, each rejecting only programs that already crashed at runtime (or relied on an unsound narrowing): v1.0.0-alpha.2's three conservative diagnostics, v1.0.0-alpha.3's four flow-narrowing invalidation fixes, and v1.0.0-alpha.4's H5 scope-blind class-unification fix (v1.0.0-alpha.5 and v1.0.0-alpha.6 add no new diagnostics — alpha.5's rewrites are opt-in or advice-only, and alpha.6's only diagnostic change is warn-level: six more secret-name keyword shapes). The behaviour changes are all in the **VM**: the v0.8.0 switch to arbitrary-precision integers, and the v0.11.0 alignment of VM value semantics with CPython (value-based dataclass equality / repr / hashing, order-independent set equality, CPython-matching float repr) — programs that relied on the old VM behaviour now compute different (correct) results. Highlights newest-first:
...
- **`from typing import List` / `Dict` / `Tuple` etc.** is *warned*, not rejected (`tyc::typing_alias_deprecated` + `tyc::typing_alias_in_annotation`, both warn-level) — use lowercase builtins.
...
- **HKT constructor parameters are `class`-header only.** `class Functor[F[_]]:` parses and unifies (v1.0.0-alpha), but `def f[F[_]](...)` and `interface F[G[_]]:` are both a `tyc::parse` error — declare the variable on a `class` and hang methods off `impl[F]`.
...
The recurring diagnostic codes and what they actually mean. **See [DIAGNOSTICS.md](DIAGNOSTICS.md) for the exhaustive 87-code reference** (89 pages live under `docs/diagnostics/` — the 87 emittable `tyc::` codes plus the `freeze` and `pub` language-reference topics, which `tyc explain` also serves) — what follows is the daily-driver subset.
...
| `tyc::missing_initialiser` (warn) | Declare-only `let`/`mut NAME: T` never assigned on any path **and** never read — a dead binding (declare-then-assign itself is legal since v0.7.0) | Initialise it, assign it before it is read, or delete the declaration |
