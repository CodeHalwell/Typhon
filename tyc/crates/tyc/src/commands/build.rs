...
fn build_source_map_v2(source_rel: &str, preprocessed: &str, line_offsets: &[usize]) -> String {
    // ⚡ Bolt: Cache newline offsets to prevent O(n²) string rescanning
    // Using partition_point (binary search) makes this O(N log K) instead of O(N*K)
    ...
    for (i, &offset) in line_offsets.iter().enumerate() {
        ...
        let n = newlines.partition_point(|&nl_offset| nl_offset < offset) as u32 + 1;
