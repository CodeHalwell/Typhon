...
pub fn run(args: BuildArgs) -> Result<()> {
    ...
    if let Ok(dty) = crate::commands::util::collect_dty_files(&src_dir) {
        for file in dty {
            ...
            if let Ok(text) = std::fs::read_to_string(&file) {
                project_shapes.entry(dotted).or_insert_with(|| {
                    /* ⚡ Bolt optimization: Use to_string_lossy to avoid Display allocation */ extract_shapes_for_path(file.to_string_lossy().as_ref(), &text)
                });
            }
        }
    }
    for (path, source) in &sources {
        ...
        project_shapes
            .entry(dotted)
            .or_insert_with(|| /* ⚡ Bolt optimization: Use to_string_lossy to avoid Display allocation */ extract_shapes_for_path(path.to_string_lossy().as_ref(), source));
    }
