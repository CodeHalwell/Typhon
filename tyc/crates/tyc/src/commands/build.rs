...
use tyc_syntax::preprocess::{
    expand_gather, expand_go, expand_lazy_imports, expand_pipes, expand_question_ops,
...
pub fn run(args: BuildArgs) -> Result<()> {
    ...
    for (path, source) in &sources {
        ...
        //   1. lazy imports → thread-safe proxy classes,
        //   2. `gather:` blocks → asyncio.TaskGroup / asyncio.gather,
        //   3. `go EXPR` → _typhon_spawn(EXPR),
        //   4. `with`-chains lower to a flat sequence of guarded unwraps,
        //   5. pipe operators rewrite `a |> f(b)` to `f(a, b)`,
        //   6. the `?` operator unwraps any remaining `Result`-typed calls.
        ...
        let expanded = expand_question_ops(&expand_pipes(&expand_with_chains(&expand_go(
            &expand_gather(&expand_lazy_imports(source)),
        ))));
...
# ── Task registry for `go` statements ─────────────────────────────────────────
# Strong references prevent the event loop's weak-ref GC from collecting tasks
# mid-flight. Each task removes itself on completion via done-callback.

_TYPHON_BACKGROUND: set = set()


def _typhon_spawn(coro):
    \"\"\"Schedule a coroutine as a background asyncio.Task with a strong ref.\"\"\"
    task = _asyncio.create_task(coro)
    _TYPHON_BACKGROUND.add(task)
    task.add_done_callback(_TYPHON_BACKGROUND.discard)
    return task
";
...
mod tests {
    ...
    #[test]
    fn build_gather_block_lowers_to_taskgroup() {
        let src = "\
...
";
    ...
    #[test]
    fn build_go_statement_lowers_to_spawn() {
        let src = "\
...
";
        ...
        let py = std::fs::read_to_string(out_dir.join("main.py")).unwrap();
        assert!(
            py.contains("_typhon_spawn(background_task())"),
            "go should lower to _typhon_spawn, got:\n{py}"
        );
        assert!(
            py.contains("handle = _typhon_spawn(other_task())"),
            "got:\n{py}"
        );
        // typhon_runtime.py must be emitted for _typhon_spawn to resolve
        assert!(
            out_dir.join("typhon_runtime.py").exists(),
            "typhon_runtime.py must be emitted when go is used"
        );
        let runtime = std::fs::read_to_string(out_dir.join("typhon_runtime.py")).unwrap();
        assert!(
            runtime.contains("def _typhon_spawn"),
            "typhon_runtime must define _typhon_spawn, got:\n{runtime}"
        );
    }
}
