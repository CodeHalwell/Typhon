...
| `ruff_python_parser` | `crates/ruff_python_parser` | Lexer + parser, plus the `val` / `var` soft-keyword support |
...
1. **`Mutability` enum + field** — `ruff_python_ast::Mutability` (`Val` /
   ...
   `comparable.rs`.
2. **`val` / `var` soft keywords** — added to `TokenKind` in
   ...
   Mid-expression occurrences of `val` / `var` continue to lex as
...
| Add `val` / `var` soft keywords + `Mutability` field             | ✅ done |
| Expose ruff parser as `tyc_syntax::ruff::parse_module`           | ✅ done |
| Port `tyc-syntax::preprocess` to stop stripping `val` / `var`    | ⏳ pending (Step 8) |
...
3. For each vendored crate, diff the upstream `src/` against the local
   ...
   - `parser/statement.rs::parse_simple_statement` val/var dispatch
