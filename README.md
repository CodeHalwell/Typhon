...
**Current release: [v0.7.0](https://github.com/CodeHalwell/Typhon/releases/tag/v0.7.0)** — the carry-over minor release that closes the Round-3 apps-feedback campaign on top of [v0.6.1](https://github.com/CodeHalwell/Typhon/releases/tag/v0.6.1) / [v0.6.0](https://github.com/CodeHalwell/Typhon/releases/tag/v0.6.0). Built a third stress round of five additional multi-file production-shaped apps (real-time game server, static site generator, vector DB, API gateway, stream processor) on top of the original ten, then turned every remaining gap into a compiler feature. Highlights since v0.6.1:

- 🟢 **`pub *` wildcard re-export aggregation in `__init__.ty`.** A package facade can now write a single `pub *` at the top of its `__init__.ty` to re-export every direct-sibling module's `pub`-marked surface. The build orchestrator collects each sibling's top-level `pub` declarations, synthesises a `from .sibling import name1, name2; …` block at the marker, and appends every aggregated name to the synthesised `__all__`. Sub-packages are included transitively (each direct sub-directory's own `pub`-marked surface plus its own `pub *` aggregation is re-exported here, cycle-safe via a `visited` set). Two new diagnostics back the feature: `tyc::pub_name_collision` (two siblings both `pub`-export the same name) and `tyc::pub_star_outside_init` (advice: `pub *` outside `__init__.ty` is a no-op).
- 🟢 **Declare-then-assign-in-arms `let NAME: T` now type-checks (R3-8).** The `let loaded: Cfg; match _load(): case Ok(v): loaded = v; case Err(e): return Err(e)` shape no longer fires `tyc::missing_initialiser`. The resolver tracks each uninitialised `let`-declaration's span; the first subsequent assignment IS the initialiser, and any second assignment fires `tyc::immutable_assign`. Sibling `match`/`if`/`elif`/`else` arms each count as a separate first-assignment path, so the natural `case Ok(v): / case Err(_):` and `if/else` shapes check clean while retaining `let` immutability outside the branching region. Companion `tyc::use_of_uninitialised` definite-assignment analysis ensures reads on paths that didn't assign are caught.
- 🟢 **`with cm() as r:` types `r` from `__enter__` / `__aenter__` and from `@contextmanager` / `@asynccontextmanager` factories (R3-3).** Three lookup paths in priority order: decorator-aware yield-type inference (pre-scans the factory body's first `yield`), concrete-class `__enter__` / `__aenter__` return types, fall-through to `Unknown`. The canonical `@asynccontextmanager async def session() -> AsyncIterator[Session]: yield Session(...)` factory shape now types as-targets correctly, where every Round-3 app previously had to type-erase to keep checking.
- 🟢 **`await` on a `Callable[..., Awaitable[T]]` / `Coroutine[Y, S, T]` unwraps to `T` (R3-1).** The canonical async-middleware shape `next: Callable[[Req], Awaitable[Resp]]` followed by `let r: Resp = await next(req)` now type-checks without a spurious `tyc::type_mismatch`. The single biggest Round-3 finding — the API gateway abandoned recursive `next`-style middleware in v0.6.1 because of this, and can adopt it now.
- 🟢 **Same-newtype arithmetic preserves the newtype (R2-12).** `newtype LogIndex = int` followed by `last_idx + 1` and `LogIndex + LogIndex` no longer widens to `Unknown`. The carve-out preserves `LogIndex` across `+ - * // % **` for same-newtype operands and `LogIndex + <literal of base>`; `/` keeps the existing widening (Python `/` is always true division). Two distinct newtypes with the same base (`LogIndex + Term`) still fire `tyc::operator_type_mismatch`.
- 🟢 **Cross-module generic method dispatch propagates class TypeVars (R3-15).** `s: Stream[int].map(f)` now records `Callable[[int], U]` as the expected parameter and returns `Stream[U]` bound at the call site. Same fix benefits field access.
- 🟢 **Resolver and syntax polish.** Nested `from X import Y` inside `if`/`for`/`while`/`with`/`try`/`match` arms now binds (R3-4); sibling `if`/`elif` branches no longer trip `no_block_shadow` for same-named `let` bindings (R3-5); multi-line `go expr(...)` calls parse (R3-2); ternary `body if test else orelse` narrows just like `if`/`else` (R3-9); `tyc::field_default_ordering` catches non-default-after-default class fields at check time (R3-11).
- 🟢 **VS Code extension `0.1.9 → 0.2.0`.** Grammar catches up to the new language surface: `pub def` / `pub async def` highlight `pub` as a storage modifier, `pub *` re-exports highlight as a single construct in `__init__.ty`, and the modifier slot accepts every combination of `pub` / `freeze` / `comptime` / `lazy` in front of `let`/`mut`/`val`/`var` and in front of `def`/`async def`.
- 🟢 **Reference apps re-organised (the "no more flat src/" pass).** Every app under `examples/apps/` now ships with grouped subdirectories under `src/` (`domain/`, `storage/`, `runtime/`, `transport/`, … per app) and a `pub *` `__init__.ty` so import sites stay short. All fifteen apps still `tyc check` clean and `tyc build` succeeds end-to-end.

Full release notes: [CHANGELOG.md](CHANGELOG.md#070--2026-05-25). v0.7.0 is strictly additive on the language surface — every previously-accepted program continues to compile to identical Python. Open frontier work (full HKT unification, general inter-procedural field-init audit) is unchanged from v0.5.0 — see [`TYPE_SYSTEM_FRONTIER.md`](TYPE_SYSTEM_FRONTIER.md) and [`docs/roadmap.md`](docs/roadmap.md).
