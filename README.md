...
- 🟢 **Ten reference apps.** `examples/apps/01-task-scheduler/` … `examples/apps/10-distributed-kv/` ship as canonical multi-file Typhon projects — each `tyc check`s clean, `tyc build`s, runs through CPython, and carries a `FRICTION.md` / README cataloguing the gaps the build surfaced (which fed the fixes above).
