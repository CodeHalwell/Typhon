...
**Current release: [v0.5.1](https://github.com/CodeHalwell/Typhon/releases/tag/v0.5.1)** — a correctness + tooling point release on top of [v0.5.0](https://github.com/CodeHalwell/Typhon/releases/tag/v0.5.0) (the post-v0.4 roadmap-sweep release). Highlights since v0.5.0:

- 🟢 **Three emit-correctness fixes (PR #120).** `tyc-format`'s in-process whitespace pass gained a triple-quote state machine so `""", encoding="utf-8")` no longer corrupts to `"utf - 8"` (Python rejects that encoding name with `LookupError`). `tyc-desugar`'s `pattern_uses_result_names` now walks every `Pattern` variant so files that only `case Ok(...)` / `case Err(...)` (without ever constructing a `Result`) trigger the auto-import of `Ok` / `Err` / `Result` from `typhon_runtime`. `tyc-syntax`'s `prepend_typhon_err_alias_import` now strips trailing newlines before its de-dupe comparison, so pipelines that introduce `?` propagation **and** `with`-chain lowering no longer ship two identical `from typhon_runtime import Err as __typhon_Err__` lines.
- 🟢 **VS Code extension grammar audit (PRs #119, #121).** ~18 TextMate miscolorings closed against ~19k lines of real Typhon code. Split `expression` into `expression` + `expression-inner` so `[...]`, `{...}`, and lambda bodies handle their own `:` (fixes `xs[1:4:2]`, `{k: v for k, v in xs}`, `lambda x: x + 1`). Balanced `#parens` matcher in expression-inner so `s: T = Depends(get_store)` consumes its own `)`. Tightened type-annotation lookahead so `if isinstance(...): return ...` and `case Foo(x): bar()` stop firing as type annotations. Extension version `0.1.5 → 0.1.7`.
- 🟢 **22 new stdlib-only examples (47–68).** Mini-app, `newtype` IDs, Fibonacci memo, linked list, BST, stack & queue, sorting, graph traversal, word frequency, state machine, iterators / generators, context managers, matrix ops, Caesar cipher, tic-tac-toe, priority queue, event bus, URL router, INI parser, rate limiter, trie, JSON-RPC builder. Each ships with both `.ty` source and an emitted `.py` companion; companions for examples 01–46 land alongside.

Full release notes: [CHANGELOG.md](CHANGELOG.md#051). v0.5.1 ships **no language-semantics change** — every previously-accepted program still compiles, and the three compiler fixes only narrow the set of *emitted-Python* bugs the toolchain produces. Open frontier work (full HKT unification, general inter-procedural field-init audit) is unchanged from v0.5.0 — see [`TYPE_SYSTEM_FRONTIER.md`](TYPE_SYSTEM_FRONTIER.md) and [`docs/roadmap.md`](docs/roadmap.md).
