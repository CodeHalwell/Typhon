...
**Current release: [v0.5.2](https://github.com/CodeHalwell/Typhon/releases/tag/v0.5.2)** — a correctness + documentation point release on top of [v0.5.1](https://github.com/CodeHalwell/Typhon/releases/tag/v0.5.1). Highlights since v0.5.1:

- 🟢 **Two compiler correctness fixes.** `tyc-syntax`'s `?`-propagation pass now disambiguates `<ident>?` in value position (`let x: T = a?`, `return a?`) from nullable type sugar — the earlier pass only recognised `f()?` (paren-prefixed) and silently rewrote `let x: T = a?` to `x: T = a | None`, which crashed at runtime with `TypeError`. Unblocks the natural `gather:` → `?` → `Ok(...)` shape used across the docs. `tyc-types`'s operator-compatibility check now accepts `set - set` and `frozenset - frozenset` (Python set difference); the sibling `&` / `|` / `^` operators already worked, only `-` had a numeric-only carve-out.
- 🟢 **Docs site sweep (~40 files).** Every complete, runnable Typhon example in the tour, types, reference, recipes, getting-started, and lowering sections is now presented as a side-by-side `<Tabs>` showing the Typhon source and the real `tyc build` output. Stale claims surfaced by the audit (the `lazy let X: T:` colon-block form, `model X frozen:`, the multi-line `|>` shape without wrapping parens, `let`-shadowing as a fix for `tyc::no_block_shadow`, the "fall-through analysis is a future release" line in `tour/control-flow.mdx`) are corrected to match what the checker actually does.
- 🟢 **Re-runnable audit harness.** `docs-site/scripts/verify_examples.py` walks every `.mdx`, classifies each code block, and tries to compile complete-program blocks via `tyc build`. `--real-only` filters partial-snippet noise; exits non-zero when real issues remain so the audit can wire into CI.

Full release notes: [CHANGELOG.md](CHANGELOG.md#052). v0.5.2 changes only the boundary of what *previously-rejected* programs the checker accepts — every previously-accepted program continues to compile to identical Python. Open frontier work (full HKT unification, general inter-procedural field-init audit) is unchanged from v0.5.0 — see [`TYPE_SYSTEM_FRONTIER.md`](TYPE_SYSTEM_FRONTIER.md) and [`docs/roadmap.md`](docs/roadmap.md).
