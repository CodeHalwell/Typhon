...
**Current release: [v0.6.0](https://github.com/CodeHalwell/Typhon/releases/tag/v0.6.0)** — an apps-feedback minor release on top of [v0.5.2](https://github.com/CodeHalwell/Typhon/releases/tag/v0.5.2). A two-round stress campaign that built ten multi-file production-shaped apps (event-sourced banking, distributed KV, mini-compiler, search engine, GraphQL server, game ECS, trading engine, ML orchestrator, web crawler, task scheduler — now under [`examples/apps/`](examples/apps/)) surfaced a batch of correctness and ergonomics gaps. Every issue from that campaign is closed in this release. Highlights since v0.5.2:

- 🟢 **Three additive features.** **(1)** `Ok` / `Err` expose the standard `Result` combinators as methods (`map`, `map_err`, `and_then`, `or_else`) so heterogeneous-error pipelines can normalise per stage in chain form (`tokenize(src).map_err(_lex)?` instead of a 4-deep `match` tower). **(2)** `impl` on a sealed-union alias (`impl Event:` where `type Event = A | B | …`) now distributes the methods to every variant class — no more `tyc::impl_unknown_class` on the natural shape, and per-variant dispatch via `match self:` still works. **(3)** New `tyc::stdlib_module_shadow` warning fires when a project `.ty` file's stem collides with a Python 3.13 stdlib top-level module (`types`, `ast`, `string`, `io`, `json`, `dataclasses`, …), preventing the baffling `ImportError` cascade where `build/types.py` on `sys.path` intercepts transitive stdlib imports.
- 🟢 **Compiler correctness sweep.** Cross-module sealed-union variant flow + interface declarations are now included in the shape extractor; cross-module function signatures now carry full `param_types` / `kwonly_types` / `return_type` (so imported `def takes(p: Price?) -> int:` looks identical to a local def at call sites, and `tyc::nullable_use` stops rendering `?` as the expected type). Exhaustive `match` on a sealed-union returned by a function call now satisfies `tyc::missing_return`; partial keyword patterns satisfy match-exhaustiveness; sibling `case` arms can bind the same `let` name; for-targets no longer rebind outer `let` bindings; `pub freeze let` parses; `pub def` is visible to the `?` operator validator; `loop.run_until_complete(coro())` and `@contextmanager` factory bodies are no longer false-positives.
- 🟢 **Better diagnostics + docs.** `tyc::type_mismatch` help text now suggests widening the annotation (`<expected> | <found>`) instead of narrowing it to the found type. New `docs/diagnostics/stdlib_module_shadow.md` catalog page with a rename table. `docs/diagnostics/class_attr_shadows_slot.md` gains a "nullary sealed-union variants" section pointing at the `pub class TyInt frozen: pass` idiom. `docs/guides/07-sealed-unions-and-match.md` documents keyword patterns as the recommended form for variants with more than two or three fields. `docs/cli.md` documents `tyc explain --list`.
- 🟢 **Fifteen reference apps.** `examples/apps/01-task-scheduler/` … `examples/apps/15-stream-processor/` ship as canonical multi-file Typhon projects — each `tyc check`s clean, `tyc build`s, runs through CPython, and carries a README cataloguing the gaps the build surfaced (which fed the fixes above). The first ten were built against v0.5.2; the additional five (real-time game server, static site generator, vector DB, API gateway, stream processor) were built against v0.6.1 in a third stress round whose findings are catalogued in [`examples/apps/TYPHON_FEEDBACK.md`](examples/apps/TYPHON_FEEDBACK.md).
- 🟢 **VS Code extension `0.1.7 → 0.1.8`.** No new grammar surface in this batch — the deep audit in v0.5.1 already covers every construct landed since. Version bump only.

Full release notes: [CHANGELOG.md](CHANGELOG.md#060--2026-05-25). v0.6.0 is strictly additive on the language surface — every previously-accepted program continues to compile to identical Python, and the new `tyc::stdlib_module_shadow` warning is non-fatal. Open frontier work (full HKT unification, general inter-procedural field-init audit) is unchanged from v0.5.0 — see [`TYPE_SYSTEM_FRONTIER.md`](TYPE_SYSTEM_FRONTIER.md) and [`docs/roadmap.md`](docs/roadmap.md).
