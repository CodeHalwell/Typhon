...
**Current release: [v0.11.0](https://github.com/CodeHalwell/Typhon/releases/tag/v0.11.0)** — VM parity sweep + `enum` keyword. A fresh adversarial stress round against v0.10.0 surfaced **22 findings** — almost entirely in the VM, with a handful of type-checker coherence gaps. This release closes every finding from the round, lands the proposed `enum` keyword as a first-class declaration form (`enum Shape: CIRCLE / SQUARE`, sugaring over `enum.Enum` with `enum.auto()` for bare members), and adds two new VM value kinds. `Value::Complex` is a real VM value with arithmetic promotion across int / float, reflected dunders, and hash support for set / dict keys. A new dict-view kind backs `dict.keys()` / `.values()` / `.items()` — they repr (`dict_keys([...])`), iterate, support `len`, are membership-testable with `in`, and are re-iterable, matching CPython. Bare `super()` is rewritten by `tyc-desugar` to the explicit two-arg form, so `@dataclass(slots=True)` (which orphans the `__class__` cell) no longer crashes; `__call__` dispatches on callable instances; `__post_init__` fires after auto-generated construction; multi-level inheritance accumulates fields across the full MRO; the generic operator-dunder handler in `binop` reaches every numeric / bitwise / matmul slot with reflected fallback. New / expanded stdlib shims: native `enum` / `datetime` (naïve / UTC) / `pathlib` (`/` join via `__truediv__`, `.parent` / `.name` / `.stem` / `.suffix` / `.suffixes` / `.parts`) / `collections.defaultdict` (factory invoked via the subscript `__missing__` hook, so `dd[k] += 1` works). Real `re.Match.group` / `.groups` / `.groupdict` capture groups, banker's `round`, `bytes` methods (`decode` / `hex` / `fromhex` / `count` / `find` / `rfind` / `startswith` / `endswith` / `split` / `strip`), `itertools.groupby(key=)`, `str.split(maxsplit=)` as a pure-keyword arg, f-string `{x=}` debug conversion, `str %` / f-string `%` runtime formatting. **VM value semantics align with CPython** — these were silent-wrong outputs under v0.10.0 and are now fixed: dataclass instance equality is value-based (with class-identity keying, not name, so cross-module same-name collisions are gone), dataclass `repr` is `Name(field=value, ...)`, dataclass instances are hashable via `HashKey::Instance`, set / frozenset equality is order-independent and repr sorts by canonical key, float `repr` matches CPython's shortest round-tripping form with scientific notation for exp < -4 or ≥ 16 (`e+NN` / `e-NN`, ≥ 2 exponent digits), `-0.0` preserved. The type checker tightens: `None` flows into `object` (matching CPython's `None: object` invariant), `str %` is type-checked against the conversion-spec shape (the same way `str.format` is), and `(5).items()` / `5["a"]` / `for x in 5:` fire at check time instead of crashing at run time. `tyc init` seeds `allow-secret-comptime = false` in the generated `typhon.toml`, `tyc-introspect` retries transient venv-fork failures with exponential backoff, and `tyc-desugar`'s decorator-list matching no longer heap-allocates per visited decorator. Full notes: [CHANGELOG.md](CHANGELOG.md#0110--2026-06-04).
