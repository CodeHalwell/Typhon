...
**Current release: [v1.0.0-alpha.6](https://github.com/CodeHalwell/Typhon/releases/tag/v1.0.0-alpha.6)** — a maintenance release on top of alpha.5, driven by the [2026-07-20 release-readiness review](docs/release-readiness-2026-07-20.md). It carries the July dependency wave safely across the `toml` 0.8 → 1.x major — fixing, before it could ship, the one regression that bump introduced (the `tyc-venv` dependency allow-list reader silently returning empty, which would have no-opped third-party arg/type checking); **widens the `tyc::contains_secret_literal` keyword table** with six squashed-acronym shapes (`API_TOKEN`, `APITOKEN`, `API_SECRET`, `APISECRET`, `API_PASSWORD`, `APIPASSWORD`), now hoisted into a single shared table so the lint and the `tyc build` scan can no longer drift; **re-pins the release pipeline's artifact download to the proven v4 line** (matching the v4 upload); and lands docs-site keyboard/reduced-motion accessibility polish, a desugar-path allocation reduction, a complete `docs/diagnostics/` index (all 87 pages), and the `.Jules`/`.jules` case-collision fix that broke fresh checkouts on macOS/Windows. **No new syntax, and no previously-*correct* program changes behaviour** — the only diagnostic-surface change is warn-level (six more secret-shaped names warned). Full notes: [CHANGELOG.md](CHANGELOG.md#100-alpha6--2026-07-21--maintenance-dependency-wave-secret-lint-keywords--release-pipeline-hygiene).

**Previous release: [v1.0.0-alpha.5](https://github.com/CodeHalwell/Typhon/releases/tag/v1.0.0-alpha.5)** — a performance-focused release on top of alpha.4. **VM performance Tier 1** (`tyc run`): a two-representation `VmInt` keeps common-path integer arithmetic allocation-free while preserving CPython's exact arbitrary-precision semantics, a per-class resolved-method cache and direct dispatch path skip redundant lookups, and slot-resolved locals replace hash-map reads for functions with no closures/`global`/`nonlocal` — compressing the VM's steady-state slowdown vs `tyc build` + CPython from ~5–18× to ~3–14× (startup-adjusted). **`[optimise]` config profile + `tyc build -O`** give a single project-wide dial that flips `auto-memoise` / `auto-gather` / `auto-parallel` / `pgo-memoise` on at once. **Seven new advice-only lints** (`tyc::perf_*` family + `lazy_import_opportunity`) flag common hot-loop anti-patterns. **A free-threading parallelisation wave** widens `auto-parallel` to comprehension filters/multi-arg/nested-pure calls, adds an integer accumulator-loop reduction (`auto-parallel-reductions`), two new advice lints, and a PEP 734 interpreters backend. **Native PEP 810 lazy imports** land on `[python] target = "3.15"` targets. **No new syntax, and no previously-*correct* program changes behaviour** — every rewrite here is opt-in or advice-only. Full notes: [CHANGELOG.md](CHANGELOG.md#100-alpha5--2026-07-09--vm-performance-tier-1-optimise-profile-perf-advice-lints--free-threading-wave).

**Previous release: [v1.0.0-alpha.4](https://github.com/CodeHalwell/Typhon/releases/tag/v1.0.0-alpha.4)** — a focused hardening pass on top of alpha.3. **Type-checker soundness:** it closes **H5**, the one HIGH finding the release-readiness review deferred — a locally declared class no longer unifies with a same-named class introspected from another module when the two have a provably different shape (a user `class Response:` no longer type-checks into an `httpx.Response`-typed slot, or vice versa). The guard is evidence-gated and degrades to the previous permissive unification on any uncertainty, so the example/stress corpus is byte-identically unchanged. **Diagnostics:** the secret-name checks in `tyc-analyse` and the `tyc build` scan match longest-first again — `APIKEY` is evaluated before the bare `KEY`, so `KEY_APIKEY` reports the more-specific suffix. **Supply-chain & packaging:** every third-party GitHub Action is SHA-pinned, the installers resolve the newest release including pre-releases (so a default install no longer silently fetches older binaries), and a round of dependency / advisory bumps lands (`crossbeam-epoch` 0.9.20 for RUSTSEC-2026-0204, plus `regex`, `memchr`, `compact_str`). **No new syntax, and no previously-*correct* program changes behaviour** — like the alpha.2 diagnostics, the H5 fix is a conservative narrowing that only rejects programs passing a provably-different-shaped class across a module boundary. Full notes: [CHANGELOG.md](CHANGELOG.md#100-alpha4--2026-07-08--post-alpha3-hardening-h5-soundness-secret-detection-correctness--supply-chain-hygiene).

**Previous release: [v1.0.0-alpha.3](https://github.com/CodeHalwell/Typhon/releases/tag/v1.0.0-alpha.3)** — a release-readiness remediation pass driven by a full-codebase review ([RELEASE_READINESS_REVIEW.md](RELEASE_READINESS_REVIEW.md)): **licensing & packaging** (repository-root MIT `LICENSE`, the upstream Ruff MIT notice vendored beside the fork, `SECURITY.md` / `CONTRIBUTING.md` / Dependabot, pre-release tags flagged as such, auto-tag gated on green CI); **type-checker fixes** (identical errors at distinct locations are all reported, nested-generic assignability is linear instead of O(2^depth), four more flow-narrowing invalidation holes closed); **six VM ↔ CPython parity fixes** (cyclic-value comparison no longer aborts the process, `str.find`-family character offsets, in-place list `+=`, float `%` / `//` sign and zero-division, broken-pipe `print`, `json.dumps` scalar-key coercion); and **tooling robustness** (256 MiB LSP stack, atomic `tyc fmt`, the `TYC_NO_INTROSPECT` kill-switch, Windows venv discovery, the `[strictness] exhaustive-match` knob actually applied). **No new syntax; no previously-*correct* program changes behaviour.** Full notes: [CHANGELOG.md](CHANGELOG.md#100-alpha3--2026-07-03--release-readiness-remediation-licensing-packaging--robustness).

**Previous release: [v1.0.0-alpha.2](https://github.com/CodeHalwell/Typhon/releases/tag/v1.0.0-alpha.2)** — the remediation of a [2026-06-28 adversarial pre-release review](docs/adversarial-review-2026-06-28.md): a sweep of type-checker **soundness** holes, **false positives** on idiomatic code, and **VM ↔ CPython** parity gaps, plus parser / config / crash-robustness hardening. The two headline soundness fixes sit on everyday code: flow narrowing of a **non-local** place (a global or instance field) is now invalidated across an intervening call or alias write — previously a narrowed `self.val` survived a method that nulled it, checking clean and crashing at runtime — and short-circuit narrowing (`x is not None and x.method()`, and the De Morgan `x is None or x.method()`) no longer false-positives on the canonical Python null-check idiom. A broad batch of typing gaps closes too: slice reads, subscript assignments, `for` / `let` tuple-unpack targets, `match` star / or-pattern captures, walrus bindings, and parameter defaults are all now typed and checked instead of silently degrading to `Unknown`. Three conservative diagnostics are added — `tyc::not_a_context_manager` (a `with` over a local class lacking the protocol), `tyc::raise_non_exception` (`raise 42` / raising a plain dataclass), and `tyc::frozen_inheritance_conflict` (mixed frozen / non-frozen dataclass bases) — each firing only where the emitted Python already crashed. Robustness: `typhon.toml` now rejects unknown keys and invalid `[checker] external` / `[strictness]` severities instead of silently reverting to defaults, and the CLI runs on a 256 MiB worker stack so deeply-nested or very long expressions no longer abort. **No new syntax, and no previously-*correct* program changes behaviour** — but unlike the purely-additive releases before it, the three new diagnostics do narrow the accepted surface for programs that already crashed at runtime (`raise 42`, raising a plain dataclass, an invalid local `with` subject, mixed frozen/non-frozen inheritance), which now surface as build-time errors instead. Full notes: [CHANGELOG.md](CHANGELOG.md#100-alpha2--2026-06-29--type-checker-soundness-sweep--vm-parity).

**Previous release: [v1.0.0-alpha](https://github.com/CodeHalwell/Typhon/releases/tag/v1.0.0-alpha)** — Typhon's **first tagged alpha**, and the first *feature-complete* milestone: the proven production surface **plus** the type-system frontier that earlier releases deferred. It rolls up milestones M1 ("Tidy & deepen") and M2 ("Type-system frontier") of the [alpha release plan](docs/alpha-release-plan.md), the early M3 polish (formatter idempotence, a performance-regression CI gate), and the `rescue` exception-boundary sugar. The frontier work now lands: **higher-kinded type unification** (a constructor variable `F` in `class Functor[F[_]]:` binds against a concrete head like `list`, with a `tyc::kind_mismatch` diagnostic on wrong arity / conflicting binding), **user-generic variance inference** (covariant / contravariant type-params inferred from usage — including across module boundaries — with `@covariant` / `@contravariant` overrides), sound variance through generic interface bounds, a **general inter-procedural field-init audit**, and 2-member non-nullable union modelling at the introspection boundary. **The production path (`tyc build` → CPython 3.13+) is stable** and carries no runtime dependency on the toolchain; the full `examples/` + `examples/apps/` + `stress/round-2026-06-21/` corpus builds to runnable Python and checks clean. As an alpha, the *surface syntax is not yet frozen* — it may change before `1.0.0` with a documented migration note. Known limitations deferred to beta: embedded `ty` Phase 2 (the Phase 1 subprocess path ships), typeshed-backed checking for pure-extension libraries, and the remaining HKT tail (function-level `F[_]` params). Additive on the accepted surface — every previously-accepted program type-checks identically. Full notes: [CHANGELOG.md](CHANGELOG.md#100-alpha--2026-06-22--first-feature-complete-alpha).

**Previous release: [v0.15.7](https://github.com/CodeHalwell/Typhon/releases/tag/v0.15.7)** — a robustness release that deepens compile-time checking of third-party code and clears a fresh batch of type-checker false positives surfaced by the 2026-06-21 stress round and a wide third-party introspection audit (43 libraries). The headline: **third-party *method* calls are now arity-checked** — a missing required argument to `PCA(n_components=2).fit()` (sklearn `fit(self, X, y=None)`) or `df.merge()` is caught at `tyc check` / `tyc build` time, the same way constructor and free-function calls already were. Three introspection-robustness fixes remove build-blockers on valid code: a re-exported proxy that raises from `inspect.signature` (Flask's `current_app`, Django's `settings`) no longer silently disables a whole module's checks; the implicit-Optional `x: T = None` idiom (real in `redis` / `pydantic`) no longer false-positives; and `pkg.sub.Thing()` multi-segment attribute calls are now checked like the `from`-import form. Three pure type-checker false positives are also fixed — `Counter + Counter` / `Counter - Counter`, a `plain class` / `class!` with a hand-written `__init__`, and a `__call__`-bearing instance passed where a `Callable` is expected — and a VM correctness gap (`str.isupper()` / `str.islower()` returned `True` on uncased strings under `tyc run`) is closed. Rounding out the release: three compiler-perf passes (fewer heap allocations in type narrowing, VM `repr`, source-map generation, and `Path` handling), a hardened hardcoded-secret lint that catches embedded secret terms, and a docs-site styling refresh. Backward-compatible — previously-accepted programs type-check identically. Full notes: [CHANGELOG.md](CHANGELOG.md#0157--2026-06-21--third-party-introspection-depth--stress-round-robustness).

**Previous release: [v0.15.6](https://github.com/CodeHalwell/Typhon/releases/tag/v0.15.6)** — a robustness release distilled from an ~198-program adversarial stress sweep ("if you can write it in Python, you can use Typhon"). It clears a cluster of type-checker false positives — several of them build-blockers on valid, idiomatic code — and closes a set of `tyc run` (VM) parity gaps so the in-process runtime matches the compiled CPython output across the whole corpus. The largest cluster was custom exception classes: `class FooError(Exception): pass` was auto-decorated with `@dataclass`, whose no-arg `__init__` broke the ubiquitous `raise FooError("msg")` at runtime; exception subclasses are now detected by base and lowered like `class!` (no dataclass), with VM parity to match. Other fixes: nested / list-star / tuple-of-union `match` exhaustiveness no longer false-fires `tyc::missing_return`; an `isinstance`-narrowed bare container is assignable to its parametric form; iterator-protocol classes conform to `Iterator[T]`; `dict` view set-operations (`d1.keys() - d2.keys()`) type-check and run; and VM `**kwargs` order, slice deletion, `__format__` dispatch, and `bytes` operators now match CPython. Two small additive features land too: flow-sensitive attribute narrowing (`if self.x is None: return …` narrows `self.x`) and a VM `abc` module shim (`from abc import ABC, abstractmethod`). Backward-compatible — previously-accepted programs type-check identically. Full notes: [CHANGELOG.md](CHANGELOG.md#0156--2026-06-16--stress-test-robustness-sweep).

**Previous release: [v0.15.5](https://github.com/CodeHalwell/Typhon/releases/tag/v0.15.5)** — a bugfix release that makes `extend BUILTIN:` work across module boundaries. Previously, builtin extension methods (e.g. `extend str: def slug(...)`) only rewrote call sites inside the declaring module — importing the module did not carry the extension, so `title.slug()` in a consumer would fire `tyc::attribute_not_found`. This release fixes the issue end-to-end: the type checker seeds sentinel class shapes from imported modules into the consumer's `ExternalShapes`; the build pass merges cross-module extension registries and injects explicit import statements for used free functions; and the VM pre-scans sibling modules for extensions before rewriting the entry module. Purely additive — previously-accepted programs are unchanged, and call sites that previously errored now resolve. Full notes: [CHANGELOG.md](CHANGELOG.md#0155--2026-06-15--cross-module-extend-builtin-propagation).

**Previous release: [v0.15.4](https://github.com/CodeHalwell/Typhon/releases/tag/v0.15.4)** — a bugfix release driven by a field report from building a layered FastAPI-style app. It closes the cross-module structural-conformance gap the reviewer called "the single biggest gap": a concrete class that reaches a consumer module only indirectly — as an imported provider's return type (`let r: Repo = get_repo()`) or behind a module-qualified annotation (`import m; r: m.Repo`) — is never seeded into the consumer's local shapes, so conformance saw zero members and wrongly fired `tyc::interface_not_conforming` (bare) or `tyc::type_mismatch` (qualified). The checker now resolves class/interface shapes through the project-wide module registry when they aren't locally seeded, so cross-module "depend on abstractions" matches same-module behaviour (and a non-conforming concrete still errors). It also fixes a `pub comptime let` / `pub comptime def` parse error (`pub` now stacks with `comptime`), and documents that `extend BUILTIN:` is module-local (unlike `extend ClassName:`, it doesn't cross imports — wrap it in a `pub def` free function instead). Additive; `tyc build` / `tyc run` output is byte-for-byte unchanged. Full notes: [CHANGELOG.md](CHANGELOG.md#0154--2026-06-15--cross-module-interface-conformance--pub-comptime-let).
