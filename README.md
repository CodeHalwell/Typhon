...
**Current release: [v0.8.1](https://github.com/CodeHalwell/Typhon/releases/tag/v0.8.1)** — a point release on top of [v0.8.0](https://github.com/CodeHalwell/Typhon/releases/tag/v0.8.0) that fixes a regression where the widened `tyc::attribute_not_found` rule false-positived on venv-introspected third-party Python classes (`uvicorn.Server.serve(...)`, `httpx.AsyncClient.aclose(...)`, `fastapi.Request.body(...)`). `InterfaceShape` now carries a `partial` flag set on every venv-derived shape; `class_hierarchy_fully_known` returns `false` whenever any class in the chain is partial. Strictly a bugfix; no language, runtime, or stdlib changes beyond the carve-out. The feature surface comes from **[v0.8.0](https://github.com/CodeHalwell/Typhon/releases/tag/v0.8.0)** on top of [v0.7.1](https://github.com/CodeHalwell/Typhon/releases/tag/v0.7.1) / [v0.7.0](https://github.com/CodeHalwell/Typhon/releases/tag/v0.7.0) — the stress-test sweep release closing **41 findings** from a multi-file v0.7.1 stress report spanning the type checker, VM, parser, lowering passes, diagnostics, and CLI. The static surface picks up several long-missing firing sites; the tree-walking VM gains arbitrary-precision integers, insertion-ordered dicts, and a much larger native stdlib; the parser learns five language-feature scaffolds the docs already advertised. Highlights since v0.7.1:
...
Full release notes: [CHANGELOG.md](CHANGELOG.md#081--2026-05-26). v0.8.0 is mostly additive on the accepted surface; the BigInt switch in the VM means programs that relied on silent i64 wrap-around now compute different (correct) results. v0.8.1 is strictly a bugfix on top of v0.8.0 — same accepted surface, the carve-out only *narrows* `attribute_not_found` to stop false-positives. Open frontier work (full HKT unification, general inter-procedural field-init audit) is unchanged from v0.5.0 — see [`TYPE_SYSTEM_FRONTIER.md`](TYPE_SYSTEM_FRONTIER.md) and [`docs/roadmap.md`](docs/roadmap.md).
