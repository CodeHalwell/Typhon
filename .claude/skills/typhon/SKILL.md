...
**Current release: v0.13.0** (2026-06-11). The language is **additive across the v0.3.0 → v0.13.0 line** — every previously-accepted program continues to type-check identically. The v0.10.0 → v0.13.0 line is dominated by two themes: making the in-process VM a true drop-in for `tyc build && python`, and deepening compile-time type-checking of third-party code. v0.13.0 also folds in a post-release code review that fixed ten correctness issues, including two VM/CPython divergences (seeded `random.sample`, and `@staticmethod`/`@classmethod` binding `self` when read through an instance) and an `incompatible_override` false positive on valid LSP-widening overrides. **One new language form** landed since v0.9.0 — the **`enum` keyword** (v0.11.0): `enum Name:` sugars over `enum.Enum`, auto-numbering bare members with `enum.auto()`. Highlights:
