...
- **Allocation reductions.** `auto_gather`'s `Candidate` / `OpportunityCandidate`, `module_all_names`, and `tyc build`'s import scan borrow `&str` from the AST instead of allocating `String`s.
