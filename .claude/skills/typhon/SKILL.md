...
- **`examples/errors/`** — 70 deliberately-broken programs covering 56 of the 87 emittable codes, one per rule, each with a header comment naming its diagnostics and showing the fix. Twelve themed directories plus `11-multi-error-programs/` (realistic files with many errors at once, and which errors mask others) and `12-known-gaps/` (programs that compile clean and still fail at runtime). Reach for these when explaining *why* a rule exists — the diagnostic is the explanation. Each file's expected codes are asserted by `tyc/crates/tyc/tests/error_examples.rs`.
