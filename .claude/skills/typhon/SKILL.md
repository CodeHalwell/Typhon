...
- **`examples/errors/`** — 73 deliberately-broken programs covering 56 of the 87 emittable codes, one per rule, each with a header comment naming its diagnostics and showing the fix. Twelve themed directories plus `11-multi-error-programs/` (realistic files with many errors at once, and which errors mask others), `12-known-gaps/` (programs that compile clean and still fail at runtime), and `13-false-positives/` (valid programs the checker wrongly rejects — consult it before trusting a surprising diagnostic). Reach for these when explaining *why* a rule exists — the diagnostic is the explanation. Each file's expected codes are asserted by `tyc/crates/tyc/tests/error_examples.rs`.
...
| `tyc::missing_initialiser` | **unreachable today** — historic code kept for `tyc explain`; declare-only `let` is legal since v0.7.0 | n/a — `use_of_uninitialised` / `immutable_assign` cover the form |
