...
- **CI.** The T0.2 differential gate ran its 16 GB runner out of memory at `--jobs 4` (swap 99.7% exhausted by the end of a ~78 s harness), which is what the intermittent `runner has received a shutdown signal` kills were; it now runs at `--jobs 2`.
