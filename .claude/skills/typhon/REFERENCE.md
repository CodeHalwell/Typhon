...
The synthesised `__all__` is emitted once at the top of the file (after imports). `pub` stacks with every modifier keyword: `pub class X frozen`, `pub model`, `pub let`, `pub mut`, `pub freeze let`, `pub newtype`, `pub interface`, `pub type`, `pub def`, `pub async def`.
