...
Env vars / flags: `TYPHON_VERSION=v0.7.0`, `TYPHON_INSTALL_DIR=/opt/typhon/bin`, or `sh install.sh --version=v0.7.0 --dir=/opt/typhon/bin`.
...
Detects arch, downloads zip + `SHA256SUMS`, verifies with `Get-FileHash`, extracts to `%LOCALAPPDATA%\Programs\Typhon\` (no admin), adds the dir to user-level `PATH`. Env vars / flags: `$env:TYPHON_VERSION = 'v0.7.0'`, `$env:TYPHON_INSTALL_DIR = 'C:\Tools\Typhon'`, or `.\install.ps1 -Version v0.7.0 -InstallDir C:\Tools\Typhon -NoPath`.
