...
- **Function-body plain assignments** (`user = find_user(1)`, `total = 0`) gain `let` on the first occurrence of each name per function, promoted to `mut` when the same name is reassigned later in the same scope. Subsequent assignments to a name already bound in the same scope are left bare (a correct re-binding). Class-body annotated assignments are left alone — those are field declarations, not locals.
