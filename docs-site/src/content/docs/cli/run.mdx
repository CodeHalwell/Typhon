...
| `--python PATH` | compile only | Python interpreter. Given explicitly it always wins; left out, the project's `.venv` is preferred, then `python3.<minor>` for `[python] target`, then `python3`. |
