...
- **~50 builtins** including `print`, `range`, `sorted`, `zip`, `enumerate`, `frozenset` (hashable as dict keys since v0.9.0), `open` (read/write/append/binary modes since v0.9.0), `@property` / `@classmethod` / `@staticmethod` / `super()` stubs (v0.9.0).
- **Native stdlib.** `math`, `os`, `sys`, `json`, `time`, `random` (v0.1.x), plus `re`, `typing`, `collections`, `functools`, `itertools`, `dataclasses`, `pathlib` (v0.8.0), plus `collections.deque`, `heapq`, `contextlib.contextmanager`, `pydantic.BaseModel` placeholder (v0.9.0).
...
- **`str` / `list` / `dict` / `set`** methods dispatched through a Rust registry; `list` also exposes `deque`'s `popleft` / `appendleft` / `extendleft` / `rotate` (v0.9.0).
...
Features the VM still doesn't support — `async` / `await` / `gather:` / `go`, real generator functions (`yield`), template strings (`t"…"`), `with` on arbitrary user `__enter__` / `__exit__` classes, the full CPython stdlib — raise `NotImplementedError` (or `ImportError` for unsupported modules) at runtime with a pointer to the responsible feature. The error message also tells you to re-run with `--compile`.
