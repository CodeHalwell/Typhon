...
```
tyc build [--out DIR] [--no-format] [--check] [--optimise]
```
...
build/
├── main.py
├── main.py.map               # source map v2
├── users.py
├── users.py.map
└── typhon_runtime/           # only if Result / go / lazy let are used
    ├── __init__.py
    ├── result.py
    ├── tasks.py
    └── lazy.py
...
If any error-severity diagnostic fires during check or analyse, the build does not produce output. The diagnostics are emitted to stderr and the process exits 2.
