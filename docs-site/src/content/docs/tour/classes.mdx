...
description: class, model, class!, impl, extend, frozen, and inheritance — the full data-modelling story.
sidebar:
  order: 5
---
...
Typhon's `class` is deliberately minimal: no `__init__`, no explicit `self`-in-body. The compiler emits a `@dataclass(slots=True)` by default, a Pydantic `BaseModel` if you use the `model` keyword, or a bare class with synthesised `__init__` if you use `class!` for framework bases. Methods live in `impl` blocks, Rust-style.
...
## `class!` — the escape hatch for framework bases
