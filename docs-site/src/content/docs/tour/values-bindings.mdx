...
Typhon refuses to infer `Any` outside an `unsafe:` region. This is stricter than TypeScript's `noImplicitAny` — there is no per-value escape hatch, only the lexical region.
