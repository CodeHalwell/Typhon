...
Typhon refuses to infer `Any` outside an `unsafe:` region. There is no per-value escape hatch.
...
Every line follows one or more rules. Once you see the pattern, you stop fighting the checker.
