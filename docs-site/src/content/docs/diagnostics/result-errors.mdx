...
The `Result` / `Ok` / `Err` method surface is now **closed**. Calling an **unknown** method on a `Result` value — a typo like `.unwarp()` — fires [`tyc::attribute_not_found`](/Typhon/diagnostics/type-errors/#tycattribute_not_found) at **check time**. Before v0.13.0 an unknown method passed the checker and crashed at runtime with `AttributeError`.
