...
### Inside another `async def` (v1.0.0-alpha.7)

The likelier mistake is forgetting `await` in code that is *already* async — and
until v1.0.0-alpha.7 that was completely silent (`tyc::async_without_await`
only covered for it by accident, when the caller had no other `await`):

```python
async def run() -> int:
    let b: int = await fetch_b()
    let d: dict[str, int] = fetch_a()   # ❌ coroutine, not dict
    return len(d) + b                   # TypeError at runtime
```

The rule here is **deliberately narrow**. It fires only when the un-awaited
call flows into a slot annotated with the coroutine's *own return type* — the
annotation on a `let` / annotated assignment, or the enclosing function's
declared return type at `return f()`. Type *equality* is required, not
assignability, so a widening `let x: object = fetch_a()` stays quiet (a
coroutine really is an `object`).

That means binding a coroutine now and awaiting it later — an intentional,
supported pattern — keeps working, because it never annotates the binding with
