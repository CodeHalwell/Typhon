...
3. **The module is untyped.** Imports return `Any` — which is `tyc::implicit_any` unless the call site is in an `unsafe:` block.
...
def f() -> int:
    let raw = messy_lib.fetch()    # ❌ tyc::implicit_any
```
