...
impl User:
    ...
    def hi() -> str:             # ❌ tyc::missing_self
        ...
```

The `self` parameter is required. Earlier docs suggested an implicit-`self` form (bare `name` resolving to `self.name`); that was never implemented because the resolver can't see the enclosing class's fields at name-resolution time. Use explicit `self.NAME`.
