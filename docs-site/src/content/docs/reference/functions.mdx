...
Type parameters are always inferred from arguments and the call-site expected type. `f[T](args)` used to crash at runtime with `TypeError: 'function' object is not subscriptable`; v0.9.0 fires a clear check-time error pointing users at the inference pattern. See [Generics](/Typhon/types/generics/) for the full reference.
