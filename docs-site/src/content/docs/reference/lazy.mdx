...
Defers the initialiser until first access. Lowers to a sentinel-cached `lazy_val(lambda: ...)` helper in `typhon_runtime`. Unlike `functools.cached_property` (instance-scoped, race-prone, writable after first eval), `lazy_val` is thread-safe and one-shot.
...
Emits as a generator function — useful when the caller may only need a prefix and doesn't need the full list materialised in memory.
