...
<Aside type="tip">
  On `[python] target = "3.15"` / `"3.15t"`, `lazy import np = numpy` skips the proxy class shown above entirely and emits the native [PEP 810](https://peps.python.org/pep-0810/) statement `lazy import numpy as np` — no `typhon_runtime` involvement. A project whose only runtime-touching feature was `lazy import` ships no generated `typhon_runtime/` package at all on that target. `3.13` / `3.14` output is unaffected — byte-for-byte unchanged — and `tyc check` / `tyc run` behave the same on every target.
</Aside>
...
- [The typhon_runtime module](/Typhon/lowering/runtime/) — `lazy_let` and the `_LazyValue` proxy.
