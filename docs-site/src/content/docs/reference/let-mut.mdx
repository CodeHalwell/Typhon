...
let mixed = [1, "two"]       # ❌ would infer list[int | str] but rejected as implicit_any
                              # because heterogeneous literals need an explicit annotation
let empty = []               # ❌ tyc::implicit_any — annotate as list[T]
```
