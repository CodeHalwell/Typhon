...
When the boundary is a one-off — a single library call whose exception you want as an `Err` — `try_result` (v0.14.6) collapses the whole `try/except` into one expression. It runs the thunk and returns `Ok(result)`, or catches an exception and returns `Err(on_err(exc))`:
...
`try_result` is a prelude name (no import needed) typed as `Result[T, E]` — `T` is the thunk's result, `E` the mapper's. The mapper is optional: `try_result(lambda: read_json(path))` returns `Result[dict, Exception]`, carrying the raw exception (map it later with `.map_err(...)`). Reach for the explicit `try/except` form when you catch *different* exception types into different errors; reach for `try_result` for the common single-boundary case.
