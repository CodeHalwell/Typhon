...
Typhon is in **active development**. The minimum-viable language — non-null types, sealed unions, `Result[T, E]`, dataclass-emit classes — is shippable today. Phase 5 (interop and developer experience) shipped in [v0.1.6](https://github.com/CodeHalwell/Typhon/releases/tag/v0.1.6), bringing `plain class`, auto-skip for framework parents, Python-semantic alignment for `or`/`and` and generators, `tyc explain` / `tyc cheatsheet`, diagnostic deep-links, and the `tyc-vm` tree-walking interpreter as the default for `tyc run`. Phase 5.5 (constructor / method arity safety) shipped in [v0.2.0](https://github.com/CodeHalwell/Typhon/releases/tag/v0.2.0): `tyc::arg_count` now fires on auto-generated class constructors and `impl` methods, cross-module shape propagation arity-checks both `from foo import Cls` and `import foo as f; f.Cls(…)`, and a new `tyc::missing_field_init` audit catches `X.__new__(X)` bypass patterns where required fields escape uninitialised. Most of the more advanced features (interfaces, comptime, lazy loading, automatic gather, free-threaded parallelism, the package-manager surface, the REPL, the debugger launcher, the migrator) are also landed; what remains is polish.
