...
- ✅ *(landed in v1.0.0-alpha)* **HKT unification** — a constructor variable `F` in `class Functor[F[_]]:` now binds against a concrete head, with `tyc::kind_mismatch` on wrong arity / conflicting binding. Still deferred: function-level `def f[F[_]]` params, non-class constructor application, and constructor composition. See [`TYPE_SYSTEM_FRONTIER.md`](https://github.com/CodeHalwell/Typhon/blob/main/TYPE_SYSTEM_FRONTIER.md) for the design sketch.
...
- ✅ *(shipped)* **`[emit] model-extra`** — `"forbid"` (default) / `"ignore"` / `"allow"` control the `ConfigDict(extra=…)` injected into every `model` class. See [`[emit]`](/Typhon/configuration/emit/).
