...
### Fixed — v0.15.2 (`as!` comment-awareness)

- ✅ **A quote inside a `#` comment no longer breaks a following `as!` / `?`.** The checked-cast preprocessor's skip mask is now built in a single unified pass, so an apostrophe (`# each field's shape`), em-dash, or `#` inside a comment or string literal no longer opens a phantom string that swallowed an `as!` or `?` on a later line (a spurious `tyc::parse` "Expected a statement"). Bugfix only — no language or API change.
