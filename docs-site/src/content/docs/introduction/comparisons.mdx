...
- **Mandatory annotations.** Every parameter and return type must be annotated. `def add(a, b): return a + b` is `tyc::missing_return_type`. Python's `mypy --strict` enforces something similar; Typhon's checker enforces it always, no opt-in.
