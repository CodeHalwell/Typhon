...
`tyc debug` is a thin v1 wrapper around `python -m pdb build/main.py`. Frames surface as `build/*.py` paths; pair with [`tyc trace`](/Typhon/cli/trace/) to remap a captured traceback back to `.ty` source via the `.py.map` sidecar. A Typhon-native source-mapping debugger is a Phase-5 deliverable. For now, the Python tooling you already have works.
