...
Typhon's collection types are Python's, with one stricter rule: **element types are mandatory**. A bare `list` (without `[T]`) is rejected by `tyc::implicit_any` wherever it is written — `let` / `mut` bindings, module bindings, class fields, **and** function parameter and return annotations; on a binding, an empty literal like `let xs = []` infers `list[Any]` and silently flows into whatever consumes it — annotate as `list[T]` to keep the type system honest.
...
def keys(d: dict) -> list:    # ❌ twice — parameter and return
    return list(d.keys())
```

Fix: spell the type parameters. The signature form is the one worth catching first — a bare `dict` there widens every call site *and* the whole function body to `Any`.
