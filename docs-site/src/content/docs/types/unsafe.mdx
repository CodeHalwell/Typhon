...
1. **Inside `unsafe:`**, expressions that would normally infer `Any` bind freely. The checker suppresses `tyc::implicit_any`, `tyc::type_mismatch`, `tyc::nullable_use`, `tyc::interface_isinstance`, `tyc::wrong_arg_count`, `tyc::not_callable`, and `tyc::non_exhaustive_match` inside the block depth.
...
- `tyc::implicit_any` — values that would otherwise infer `Any` bind freely.
...
- `tyc::wrong_arg_count` — call-site arity mismatches are tolerated.
