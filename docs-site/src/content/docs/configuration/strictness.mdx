...
| `nullable-use` | `"error"` \| `"warn"` \| `"off"` | `"warn"` | Severity for the *attribute-rooted* form of [`tyc::nullable_use`](/Typhon/diagnostics/nullability/) — dereferencing a possibly-`None` field (`self.db.host` where `db: Db?`). Introduced at `"warn"` because the check never ran before v1.0.0-alpha.7; promote to `"error"` once your code is clean. The bare-name form is always an error and is not governed by this knob. |
...
| `require-with` | `"error"` \| `"warn"` \| `"off"` | `"warn"` | Severity for [`tyc::resource_not_managed`](/Typhon/diagnostics/compile-errors/) — a call to a known resource-returning function (`open`, `socket.socket`, `sqlite3.connect`, `tempfile.*`) bound to a variable outside a `with` statement. `"error"` promotes it once a codebase has paid down the migration; `"off"` drops it. |
| `blocking-in-async` | `"error"` \| `"warn"` \| `"off"` | `"warn"` | Severity for [`tyc::blocking_in_async`](/Typhon/diagnostics/async-errors/) — a direct call to a known-blocking stdlib function (`time.sleep`, `requests.get`, `subprocess.run`, `socket.recv`, …) inside an `async def`. `"error"` promotes it for codebases that have finished moving to async-aware libraries; `"off"` suppresses it. |
| `stub-check` | `"error"` \| `"warn"` \| `"off"` | `"error"` | Severity for [`tyc::stub_mismatch`](/Typhon/diagnostics/stub-errors/) from `tyc check --stubs` — a `.dty` stub whose surface diverges from its sibling `.ty` / `.py` implementation. `"error"` (default) fails the check; `"warn"` surfaces drift without blocking CI; `"off"` drops the findings. |
...
| `suggest-gather` | bool | `true` | Emit the advice-level [`tyc::gather_opportunity`](/Typhon/diagnostics/async-errors/#tycgather_opportunity-advice) hint when 2+ adjacent independent awaited calls could be wrapped in a `gather:` block. Never rewrites, never blocks a build. |
| `suggest-perf` | bool | `true` | Emit the advice-level `tyc::perf_*` micro-optimisation family (six lints) plus [`tyc::lazy_import_opportunity`](/Typhon/pitfalls/performance/#automatic-performance-advice). Never rewrites, never blocks a build. |
...
Bodies of `@contextmanager` / `@asynccontextmanager` factories are exempt — acquiring the resource is their job.

## `blocking-in-async`

Severity for `tyc::blocking_in_async`: a direct call to a known-blocking stdlib function (`time.sleep`, `requests.get` / `post` / …, `urllib.request.urlopen`, `subprocess.run` / `call` / `check_output`, `input`, `socket.recv`) inside an `async def` stalls the event loop.
...
Severity for `tyc::stub_mismatch`, produced by [`tyc check --stubs`](/Typhon/cli/check/) when a `.dty` stub's surface (missing-in-implementation / missing-in-stub / signature mismatch) diverges from its sibling `.ty` or `.py` implementation.
...
**Off by default.** When on, every function the analyser can *prove* pure — with immutable, hashable parameters and an immutable return type — gets `@functools.cache`. The runtime impact:
...
<Aside type="caution">
  Even when `auto-memoise = true`, the analyser only caches what it can prove: a call it cannot classify (a method on a value of unknown type, a module outside the pure stdlib allow-list) leaves the function un-cached, and a function returning a `list`, an iterator or a mutable object is never cached because every caller would share one object. See [`@pure` and `@memo`](/Typhon/reference/pure-memo/#what-pure-proves).
</Aside>
...
A handful of advisory lints fire at **warn** level without a dedicated `[strictness]` knob — they ride the project's global severity rather than being individually configurable. They're listed here so you know they exist; the full semantics live in the [diagnostics catalog](/Typhon/diagnostics/reading/).
